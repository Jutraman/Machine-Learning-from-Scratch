...
elseif nargin == 0
    fprintf(1, "Please input your tree!")
    return;
end
...
if ~isempty(tree.kids)
    ...
    opText = tree.op;
    text(tree.X, tree.Y,num2str(opText),'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom','interpreter', 'none', 'FontSize', 10);
else
    line(tree.X, tree.Y, 'marker', '.', 'markersize', 8)
    text(tree.X, tree.Y, num2str(tree.prediction) ,'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom', 'interpreter', 'none', 'FontSize', 10);
end
